        ...
        if (_.isArray(expected)) {
            ...
            _.each(expectedKeys, (expectKey) => {
                if (typeof actual[expectKey] == typeof expected[expectKey]) {
                    if (me._isJSON(actual[expectKey])) {
                        assertion = assertion && me._assert(actual[expectKey], expected[expectKey], key + "." + expectKey, ignoreDiff, isType)
                    } else if (_.isArray(actual[expectKey])) {
                        assertion = assertion && me._assertArray(actual[expectKey], expected[expectKey], key + "." + expectKey, isType);
                    } else {
                        assertion = assertion && me._chaiDeepEqual(actual[expectKey], expected[expectKey], key + "." + expectKey, ignoreDiff, isType);
                    }
                } else {
                    assertion = assertion && me._chaiDeepEqual(actual[expectKey], expected[expectKey], key + "." + expectKey, ignoreDiff, isType);
                }
            ...
            if (!me.jsonContains) {
                _.each(missingKeysInActual, function (missingKey) {
                    assertion = assertion && me._chaiDeepEqual(actual[missingKey], expected[missingKey], key + "." + missingKey, ignoreDiff, isType);
                })
            }
            return assertion;
        } else {
