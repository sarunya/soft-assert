        ...
        if (expectedArr.length > 0 && me._isJSON(expectedArr[0])) {
            ...
            for (let i = 0; i < expectLen; i++) {
                for (let j = 0; j < actualLen; j++) {
                    try {
                        if (me._assert(actualArr[j], expectedArr[i], key + "." + j, false)) {
        ...
        if (_.isArray(expected)) {
            ...
            _.each(expectedKeys, (expectKey) => {
                if (typeof actual[expectKey] == typeof expected[expectKey]) {
                    if (me._isJSON(actual[expectKey])) {
                        assertion = assertion && me._assert(actual[expectKey], expected[expectKey], key + "." + expectKey)
                    } else if (_.isArray(actual[expectKey])) {
                        assertion = assertion && me._assertArray(actual[expectKey], expected[expectKey], key + "." + expectKey);
                    } else {
                        assertion = assertion && me._chaiDeepEqual(actual[expectKey], expected[expectKey], key + "." + expectKey, ignoreDiff);
                    }
                } else {
                    assertion = assertion && me._chaiDeepEqual(actual[expectKey], expected[expectKey], key + "." + expectKey, ignoreDiff);
                }
            ...
            if (!me.jsonContains) {
                _.each(missingKeysInActual, function (missingKey) {
                    assertion = assertion && me._chaiDeepEqual(actual[missingKey], expected[missingKey], key + "." + missingKey, ignoreDiff);
                })
            }
            return assertion;
        } else {
            return me._chaiDeepEqual(actual, expected, key);
        }
        return true;
    }
